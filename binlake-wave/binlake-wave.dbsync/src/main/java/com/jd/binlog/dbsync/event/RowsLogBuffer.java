...
import com.jd.binlog.dbsync.JsonConversion;
...
public final class RowsLogBuffer {
     ...
     */
    public static int mysqlToJavaType(int type, final int meta, boolean isBinary) {
        int javaType;
    ...
    final Serializable fetchValue(int type, final int meta, boolean isBinary) {
        ...
        switch (type) {
            ...
            case LogEvent.MYSQL_TYPE_DECIMAL: {
                 ...
                 */
                logger.debug("MYSQL_TYPE_DECIMAL : This enumeration value is "
            ...
            case LogEvent.MYSQL_TYPE_NEWDATE: {
                 ...
                 * and cannot exist in a binlog.
                 */
                logger.debug("MYSQL_TYPE_NEWDATE : This enumeration value is "
                            + "only used internally and cannot exist in a binlog!");
                javaType = Types.DATE;
                value = null; /* unknown format */
                length = 0;
            ...
            case LogEvent.MYSQL_TYPE_ENUM: {
                ...
                // logger.debug("MYSQL_TYPE_ENUM : This enumeration value is "
                // + "only used internally and cannot exist in a binlog!");
            ...
            case LogEvent.MYSQL_TYPE_TINY_BLOB: {
                 ...
                 * and cannot exist in a binlog.
                 */
                logger.debug("MYSQL_TYPE_TINY_BLOB : This enumeration value is "
                            + "only used internally and cannot exist in a binlog!");
            }
            case LogEvent.MYSQL_TYPE_MEDIUM_BLOB: {
                /*
                 ...
                 * and cannot exist in a binlog.
                 */
                logger.debug("MYSQL_TYPE_MEDIUM_BLOB : This enumeration value is "
                            + "only used internally and cannot exist in a binlog!");
            }
            case LogEvent.MYSQL_TYPE_LONG_BLOB: {
                /*
                 ...
                 * and cannot exist in a binlog.
                 */
                logger.debug("MYSQL_TYPE_LONG_BLOB : This enumeration value is "
                            + "only used internally and cannot exist in a binlog!");
            }
            case LogEvent.MYSQL_TYPE_BLOB: {
                /*
            ...
            case LogEvent.MYSQL_TYPE_JSON: {
                ...
                if (0 == len) {
                    ...
                    JsonConversion.Json_Value jsonValue = JsonConversion.parse_value(buffer.getUint8(), buffer, len - 1, charsetName);
            ...
            case LogEvent.MYSQL_TYPE_GEOMETRY: {
                ...
                /* debuging unsupport cloumn type */
                logger.debug(String.format("!! Unsupport column type MYSQL_TYPE_GEOMETRY: meta=%d (%04X), len = %d",
                    meta,
                    meta,
                    len));
                javaType = Types.BINARY;
    ...
    public final LogBuffer getBuffer() {return buffer; }
